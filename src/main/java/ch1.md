...
인지를 받는 객체는 서로의 연산을 요청할 수도 있지만 서로에 대해 책임은 지지 않습니다.
인지 관계는 통합 관계보다 관련성이 약해서 객체들 사이의 결합도가 약합니다.

### 변화에 대비한 설계(p.55)
재사용을 최대화하기 위해서는 새로운 요구 사항과 기존 요구 사항에 발생한 변경을 예측하여 앞으로의 시스템 설계가 진화할 수 있도록 해야 합니다.
변화를 수용하지 못하는 설계는 앞으로 재설계가 필요하게 됩니다.

디자인 패턴은 다른 부분에 독립적으로 시스템 구조를 변경할 수 있게 하여, 시스템이 어떤 특정 변화에 순응할 수 있도록 합니다.

아래는 디자인 패턴을 써서 재설계를 하지 않았을 때 발생하는 문제들입니다.
1. 특정 클래스에서 객체 생성
   1. 클래스 이름을 명시하여 인스턴스를 만들면, 구체 클래스에 의존적(종속)이게 됩니다. 이러한 종속은 변화를 수용하기 어렵습니다.
2. 특정 연산에 대한 의존성
   1. 특정 연산을 선택하여 사용하면, 요청을 만족하는 한가지의 작업만 수행할 수 있습니다.
3. 하드웨어와 소프트웨어 플랫폼에 대한 의존성
   1. 기존에 존재하는 시스템 인터페이스와 응용프로그래밍 인터페이스는 소프트웨어 및 하드웨어 플랫폼마다 모두 다릅니다.
   2. 특정 플랫폼에 종속되어있다면 다른 플랫폼으로의 이식도 어렵습니다.
   3. 버전변경의 어려움도 있습니다.
4. 객체의 표현이나 구현에 대한 의존성
   1. 사용자가 객체의 표현이나 구현에 대해 모든 방법을 알고있고 이를 사용하고있다면, 객체의 변경이 사용자의 변경을 초래할 수 있습니다.
   2. 이런 불필요한 공개 정보는 감추어야합니다.
5. 알고리즘 의존성
   1. 알고리즘에 종속된 객체라면 알고리즘이 변할 때 마다 객체도 변경해야 합니다.
   2. 변경이 발생할 수 있는 알고리즘은 분리해내는 것이 바람직합니다.
6. 높은 결합도
   1. 높은 결합도를 갖는 서로간 클래스들은 재사용되기 어렵습니다. 높은 결합도는 결국 커다란 시스템을 만들어버립니다.
   2. 그러면 변경이 발생하였을 때, 클래스를 수정하기 위해 전체를 이해하고 있어야하고, 유지보수가 어렵습니다.
7. 서브클래싱을 통한 기능 확장
   1. 서브클래스를 정의하려면, 최상위 클래스부터 자신이 속한 직속 부모 클래스까지 모든 것을 이애하고 있어야합니다.
8. 클래스 변경이 편하지 못한 점
   1. 소스 코드가 필요한데 없거나, 어떤 변경을 하면 기존 서브클래스의 다수를 수정해야할 수 있습니다.

디자인 패턴은 위의 예시를 보다 쉽게 해결할 수 있도록 지원합니다.

#### 프레임워크(p.59)
프레임워크는 특정한 부류의 소프트웨어에 재사용성을 부여하여 개발할 수 있도록 만들어 주는 관련 클래스들의 집합입니다.

프레임워크의 재정의란 프레임워크에 정의한 클래스를 상속받아 특정 응용프로그램을 지원하는 서브클래스를 정의하는 것을 뜻합니다.

프레임워크는 응용프로그램에 대한 뼈대를 제공합니다.
즉, 프레임워크는 클래스와 객체들의 분할, 전체 구조, 클래스와 객체들 간의 상호작용, 객체와 클래스 조합방법, 제어 흐름에 대해 미리 정의합니다.

디자인 패턴과 프레임워크는 여러가지 공통점을 가지고 있지만, 대표적으로 아래 세가지의 차이가 있습니다.
...
## 1.7 디자인 패턴을 고르는 방법(p.62)

총 20가지가 넘는 디자인 패턴 중 어떤 디자인 패턴을 적용해야하는지에 대한 접근 방법에 대해 살펴봅시다.
...
- 재설계의 원인을 파악합시다.
- 설계에서 가변성을 가져야하는 부분이 무엇인지 파악합시다.
